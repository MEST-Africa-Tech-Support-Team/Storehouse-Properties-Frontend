    ...
    <div className="space-y-6">
      ...
      <section className="bg-white p-4 rounded-xl border border-[#E5E7EB] shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-center gap-4">
          ...
          <div className={`${showFilters ? 'flex' : 'hidden'} lg:flex flex-col lg:flex-row items-stretch lg:items-center gap-3 animate-in fade-in slide-in-from-top-2 duration-200`}>
            <FilterSelect label="Booking Status" options={[{ value: "confirmed", label: "Confirmed" }, { value: "canceled", label: "Canceled" }]} />
            <FilterSelect label="Payment Status" options={[{ value: "paid", label: "Paid" }, { value: "refunded", label: "Refunded" }]} />
            <FilterSelect label="Children" options={[{ value: "yes", label: "Children: Yes" }, { value: "no", label: "Children: No" }]} />
          ...
          <table className="w-full text-left border-collapse min-w-[1100px]">
            <thead>
              <tr className="bg-[#F9FAFB] border-b border-[#E5E7EB]">
                {["Booking ID", "Property", "Customer", "Dates", "Guests", "Children", "Status", "Payment", "Amount", "Actions"].map((head) => (
                  <th key={head} className="px-6 py-4 text-[11px] font-bold text-[#6B7280] uppercase tracking-wider">
                    {head}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-[#E5E7EB]">
              {bookingsData.map((booking) => (
                <tr key={booking.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-4 text-sm font-bold text-[#1e5eff]">{booking.id}</td>
                  <td className="px-6 py-4">
                    <div className="font-bold text-[#1a1a1a] text-sm">{booking.property}</div>
                    <div className="text-xs text-[#6B7280]">{booking.location}</div>
                  </td>
                  <td className="px-6 py-4 text-sm font-medium text-[#1a1a1a]">{booking.customer}</td>
                  <td className="px-6 py-4">
                    <div className="text-[13px] text-[#1a1a1a] font-medium">{booking.checkIn}</div>
                    <div className="text-xs text-[#6B7280] flex items-center gap-1">
                      <RiArrowRightLine size={12} /> {booking.checkOut}
                    </div>
                  </td>
                  <td className="px-6 py-4 text-xs font-medium text-[#1a1a1a]">{booking.guests}</td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-md text-[10px] font-bold ${
                      booking.children === "YES" ? "bg-[#1E5EFF] text-white" : "bg-[#F3F4F6] text-[#1a1a1a]"
                    }`}>
                      {booking.children}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${
                      booking.status === "confirmed" ? "bg-[#DCFCE7] text-[#15803D]" : "bg-[#FEE2E2] text-[#B91C1C]"
                    }`}>
                      {booking.status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${
                      booking.payment === "paid" ? "bg-[#DCFCE7] text-[#15803D]" : "bg-[#F3F4F6] text-[#1a1a1a]"
                    }`}>
                      {booking.payment}
                    </span>
                  </td>
      ...
      <div className="px-6 py-5 bg-white rounded-xl border border-[#E5E7EB] flex flex-col sm:flex-row items-center justify-between gap-4">
        <p className="text-sm text-[#6B7280] font-medium text-center sm:text-left">
          Showing <span className="text-[#1a1a1a] font-bold">{bookingsData.length}</span> of <span className="text-[#1a1a1a] font-bold">247</span> bookings
        </p>
